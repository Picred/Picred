# 👋 Hi, I'm Andrei Stefan

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Roboto+Mono&size=42&pause=1500&color=CD0000&background=FF000000&center=true&vCenter=true&width=1200&height=70&lines=Software+Engineer;Clean+Code;Software+Development+Life+Cycle;Software+Design+%2F+Architecture)](https://git.io/typing-svg)

## 📌 About Me
_Passionate about building clean, maintainable, and efficient web applications. I bridge academic software engineering practices with hands-on development to solve real-world problems._

- **Master's Student** in Software Engineering @ [Politecnico di Torino](https://www.polito.it/)
- Focus on **Software Engineering** & **Full-Lifecycle Application Development**
- Experienced with **Git**, **Linux** environments, and building automated, testing-backed solutions
- Open to collaborations on **Open Source**, **Web Platforms**, and innovative **Software Development** projects

<p align="left">
  <img src="https://komarev.com/ghpvc/?username=Picred&label=Profile%20views&color=0e75b6&style=flat" alt="Profile views" />
  <img src="https://img.shields.io/github/stars/picred?style=social" alt="Github Stars" />
</p>

---

## 🚀 Projects

### 🔥 Highlighted Projects
*   🚂 [Race The Rails](https://github.com/Picred/race-the-rails) – Full-stack client-server SPA featuring a dedicated frontend, REST APIs, and secure session authentication.
*   🛒 [EZShop](https://github.com/Picred/EZShop) – API application designed to support managers of small retail shops with multi-cash register management.
*   📟 [Tetris](https://github.com/Picred/tetris-landtiger) – Fully functional Tetris game implemented on a LandTiger LPC-1768 board using ARM assembly and C, featuring malus and power-ups.
*   🗺️ [Maze Navigation](https://github.com/Picred/maze-navigation) *(Contributor)* – Academic research framework for navigation strategies in mazes (Thesis project).

### 🛠️ Other Projects
*   🌐 [Cineaura](https://github.com/Picred/cineaura) – Web app for managing and discovering movies. ([Demo](https://picred.github.io/cineaura))
*   👨🏻‍💻 [Linux Terminal](https://github.com/Picred/linux-terminal-sim) – Interactive Linux Terminal simulator running directly in the browser. ([Demo](https://picred.github.io/linux-terminal-sim/))
*   🧠 [Brain Test Bot](https://github.com/Picred/brain-test-bot) – Telegram chatbot for interactive quizzes and cognitive testing.
*   ⏭️ [Granimal](https://github.com/Picred/granimal) *(Contributor)* – A platform designed to keep track of your favorite anime.
*   📟 [Air Quality Monitor](https://github.com/Picred/air-quality-monitor) – Real-time air quality monitoring for selected cities using public APIs.
*   ☁️ [Local Chat](https://github.com/Picred/local-chat) – Lightweight local network chat application.
*   📚 [Database Cinema Project](https://github.com/Picred/cinema-database) – Relational database design and implementation for cinema management.
*   🎮 [Hangman Game](https://github.com/Picred/hangman-game) – A classic Hangman game implemented in Python.

---

## 🛠️ Tech Stack & Skills

### 💡 Languages
[![Languages](https://skillicons.dev/icons?i=rust,bash,js,ts,py,java,c,cpp,php,processing)](https://skillicons.dev)

### 🎨 Frontend
[![Frontend](https://skillicons.dev/icons?i=react,vite,html,css,tailwind,bootstrap,daisyui)](https://skillicons.dev)

### ⚙️ Backend & DevOps
[![Backend](https://skillicons.dev/icons?i=laravel,docker,git,mysql,sqlite,mongodb,nodejs,linux)](https://skillicons.dev)

### 📊 Data & ML
<!-- [![Data & ML](https://skillicons.dev/icons?i=py,jupyter,kafka,elasticsearch)](https://skillicons.dev) *(Apache, Spark, Numpy)* -->
<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/numpy/numpy-original.svg" width="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/jupyter/jupyter-original.svg" width="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/apachekafka/apachekafka-original.svg" width="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/apache/apache-original.svg" width="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/apachespark/apachespark-original.svg" width="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/elasticsearch/elasticsearch-original.svg" width="40"/>
</p>

---

## 📫 Connect with me

<p align="left">
  <a href="https://picred.github.io" target="_blank">
    <img src="https://img.shields.io/badge/Portfolio-0078D4?style=for-the-badge&logo=microsoft-edge&logoColor=white" alt="Portfolio" />
  </a>
  <a href="https://www.linkedin.com/in/andreistefand/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
</p>

---

### 💬 Daily Quote
<p align="center">
  <a href="https://github.com/piyushsuthar/github-readme-quotes"> 
    <img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight" alt="Quotes" /> 
  </a>
</p>

